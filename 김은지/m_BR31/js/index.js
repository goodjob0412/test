...
let sp = ["iphone","ipad"];
    ...
    for(let i=0; i<sp.length; i++) {
        if(userAgent.match(sp[i])) {
            // console.log('아이폰')
            $('.br_background').css({backgroundAttachment: 'unset'})
        } 
    }
...
//아이스크림 이미지 여러개 배치
for(let i=2; i<13; i++) {
    let best_img = `<img class="best" src="./img/item/icecream/${i}.png" alt="best" style="opacity: 0;">`; 
...
//박스 애니메이션
let menu_top = $('.sec3').offset().top;
// console.log(`menu_top:${menu_top}`)
let menu_height = $('.sec3').outerHeight();

$(window).scroll(function(){
    let w_top = $(window).scrollTop();
    let m_amount = '50px'
    if(w_top >= menu_top - 200){
        // console.log(1111)
